...
// 2) Define the review form state type
interface ReviewFormState {
  status: 'PENDING' | 'APPROVED' | 'REJECTED' | 'CHANGES_REQUESTED';
  technicalPass: boolean;
  contentPass: boolean;
  notes: string;
  vrcResults: Record<VRCKey, boolean>;
}
...
export default function AdminReviewPage({ params }: { params: { id: string } }) {
  ...
  // 3) Type key as VRCKey
  const handleVRCChange = (key: VRCKey) => {
    setReviewForm((prev) => ({
      ...prev,
      vrcResults: {
        ...prev.vrcResults,
  ...
  // 4) Strongly type the labels map
  const vrcLabels: Record<VRCKey, string> = {
        ...
        >
            ...
            >
              ...
              >
                <div>
                  ...
                  <div style={{ fontWeight: 600 }}>
                    {(Number(app.sizeBytes) / (1024 * 1024)).toFixed(2)} MB
                  </div>
                </div>
                <div>
                  <div
          ...
          >
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              >
                VRC Compliance Check
              </div>
              {(
                Object.entries(vrcLabels) as [VRCKey, string][]
              ).map(([key, label]) => (
                <label
                  key={key}
                  style={{
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    setReviewForm({
                      ...reviewForm,
                      technicalPass: e.target.checked,
                    })
                  }
                  style={{ width: 18, height: 18, cursor: 'pointer' }}
                />
              ...
              <select
                ...
                onChange={(e) =>
                  setReviewForm({
                    ...reviewForm,
                    status: e.target
                      .value as ReviewFormState['status'],
                  })
                }
                style={{
                  width: '100%',
            ...
            {reviewForm.status === 'REJECTED' && (
              ...
              >
                ⚠️ Rejecting this app will suspend it from the store.
                Make sure to provide detailed feedback.
              </div>
            )}
          </div>
        </div>
